...
module.exports = {
  data: new SlashCommandBuilder().setName("history").setDescription("View action history for a player").addStringOption((option) => option.setName("user").setDescription("Username or ID").setRequired(true)),
  async execute(interaction, exports2) {
    ...
    const history = await exports2[EasyAdmin].getActionHistory(user.identifiers);
    if (!history || history.length === 0) {
      let embed2 = await prepareGenericEmbed(`No action history found for **${user.name}**.`);
    ...
    for (const entry of entries) {
      const date = new Date(entry.time * 1e3).toUTCString();
