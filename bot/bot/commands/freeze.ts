...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction): Promise<void> {
		...
		const ret = await shared.ea().freezePlayer(user.id, true)

		if (ret) {
			const embed = await shared.prepareGenericEmbed(`Successfully froze **${user.name}**.`)
			await interaction.reply({ embeds: [embed!] })
		} else {
			const embed = await shared.prepareGenericEmbed(`Could not freeze **${user.name}**.`)
			await interaction.reply({ embeds: [embed!] })
		}
	},
}
