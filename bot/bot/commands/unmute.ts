...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction): Promise<void> {
		...
		const ret = await shared.ea().mutePlayer(user.id, false)

		if (ret) {
			const embed = await shared.prepareGenericEmbed(`Successfully unmuted **${user.name}**.`)
			await interaction.reply({ embeds: [embed!] })
		} else {
			const embed = await shared.prepareGenericEmbed(`Could not unmute **${user.name}**.`)
			await interaction.reply({ embeds: [embed!] })
		}
	},
}
