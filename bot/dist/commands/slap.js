// bot/commands/slap.js
module.exports = {
  data: new SlashCommandBuilder().setName("slap").setDescription("Substracts amount of HP from player").addStringOption((option) => option.setName("user").setDescription("Username or ID").setRequired(true)).addIntegerOption((option) => option.setName("amount").setDescription("Amount of HP to slap the user for.").setRequired(true)),
  async execute(interaction, exports2) {
    ...
    var ret = exports2[EasyAdmin].slapPlayer(user.id, slapAmount);
    if (ret) {
