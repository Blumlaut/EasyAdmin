...
module.exports = {
  data: new SlashCommandBuilder().setName("kick").setDescription("Kicks a User").addStringOption((option) => option.setName("user").setDescription("Username or ID").setRequired(true)).addStringOption((option) => option.setName("reason").setDescription("Reason Text").setRequired(true)),
  async execute(interaction, exports2) {
    ...
    DropPlayer(user.id, sprintf(exports2[EasyAdmin].GetLocalisedText("kicked"), interaction.user.tag, reason));
