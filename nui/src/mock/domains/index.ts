/**
 * Barrel export for all domain mocks.
 */

export { playersMock, DEMO_PLAYERS, DEMO_PERMISSIONS, DEMO_CACHED } from './players.mock'
export { bansMock, DEMO_BANS } from './bans.mock'
export { reportsMock, DEMO_REPORTS } from './reports.mock'
export { statisticsMock } from './statistics.mock'
export { resourcesMock } from './resources.mock'
export { dashboardMock } from './dashboard.mock'
export { serverMock } from './server.mock'
export { settingsMock } from './settings.mock'
export { profilerMock } from './profiler.mock'
export { pluginsMock } from './plugins.mock'
export { mapMock, startMapStreamer } from './map.mock'
