...
nui/src/
  ...
  pages/                      # NEW: top-level views
    ...
    Players/
      PlayerListPage.tsx      # player list + search
      PlayerDetailPage.tsx    # player info + actions
    ...
    Server/
      ServerPage.tsx          # server management actions
      AnnouncementModal.tsx   # announcement prompt
      ResourceModal.tsx       # start/stop resource prompt
      ConvarModal.tsx         # set convar prompt
      CleanupModal.tsx        # clean area config
    Settings/
      SettingsPage.tsx        # all settings options
...
4. **Raw permissions** -- `Record<string, boolean>` passed from Lua, no hardcoded lists
5. **No global state library** -- React `useState`/`useReducer` in `App.tsx` is sufficient for this scale
6. **Type safety** -- all Lua callbacks typed with `callLua<T>(action, data)`
7. **Permission-gated UI** -- components check `permissions[key]` and hide/disable accordingly
...
- `KeyValueTable` -- display rows of key-value pairs (ban details, player info).
...
**Testing:** Unit tests for `InputPrompt`, `Pagination`, `BanDurationPicker`.
...
**Deliverables:**
- `pages/Bans/BanListPage.tsx` -- paginated list of bans (10 per page), search by ban ID/name/reason/identifier.
- `pages/Bans/BanDetailPage.tsx` -- ban overview: ban ID, reason, name, banner, expire, identifiers. Editable fields when `player.ban.edit`. Save changes button.
...
**Lua callbacks (gui_nui.lua):**
- `requestBanList` -- fetch ban list from server.
- `searchBanList` -- search bans (can be client-side if full list is cached).
- `editBan` -- send edited ban data to server.
- `unbanPlayer` -- remove ban by ban ID.
- Event: `updateBanList` -- push updated ban list to frontend.
...
**Deliverables:**
- `pages/Reports/ReportListPage.tsx` -- list of open reports with color coding (yellow = normal, red = emergency, green = claimed).
...
**Lua callbacks:**
- `requestReports` -- fetch reports from server.
...
**Deliverables:**
- Global player search (searches online + cached players, shows results in a dropdown).
- "All Players" actions section (teleport everyone to me).
- Enhanced `PlayerDetailPage`:
  - Kick with `InputPrompt` for reason.
  - Ban with `InputPrompt` for reason + `BanDurationPicker`.
  - Warn with `InputPrompt` for reason.
  - Teleport sub-options (to player, player to me, me back, player back, into closest vehicle).
  - Slap with slider (1-20x10 damage).
  - Routing bucket (join / force) -- conditional on `ea_routingBucketOptions` convar.
  - Copy Discord to clipboard.
  - Player info panel (identifiers, IP, Discord, health, etc.).
...
**Deliverables:**
- `pages/Server/ServerPage.tsx` -- organized sections:
  - **Announcements** -- `InputPrompt` for message text.
  - **Server Info** -- set gametype, set mapname (each with `InputPrompt`).
  - **Resources** -- start resource, stop resource (with self-protection: can't stop EasyAdmin itself).
  - **Convars** -- set convar (two-step: name then value, two `InputPrompt` modals).
  - **Cleanup** -- `CleanupModal` with type selector (cars/peds/props), radius selector, deep clean toggle.
...
**Deliverables:**
- `pages/Settings/SettingsPage.tsx` -- organized sections:
  - **Data** -- refresh ban list, refresh cached players, refresh permissions.
  - **Display** -- ban list show type (reasons vs licenses), menu orientation (left/middle/right).
  - **Privacy** -- anonymous mode toggle.
  - **Accessibility** -- screen reader / TTS toggle + speed selector.
  - **Appearance** -- force easter egg (FiveM only, disabled on RedM).
...
- RedM-specific UI adjustments (hide cleanup, adjust player list for non-infinity mode).
...
- Plugin menu hooks (Lua calls `SendNUIMessage` to inject custom menu items).
...
client/
  ...
  nui/
    ...
    bans.lua                  # ban list, search, edit, unban
...
5. **Callbacks follow the same pattern** -- validate permission, execute, notify frontend, return result
